    ...
    function(search, record, email, runtime, log, file, task, config) {
        function execute(context) {
            ...
            var fileCabinetId = scriptObj.getParameter({
                name: 'custscript_pe_filecabinet_sunat_id'	
            }); //223

            try {
                log.debug({
                    title: 'INICIO',
                    details: 'INICIANDO SCHEDULED SUNAT: ' + fedIdNumb + ' - ' + fedIdNumb
                });
                ...
                log.debug({
                    title: 'myObj',
                    details: 'FINALIZANDO DETRACCIONES SUNAT, FILE GENERADO ' + fileId
                });
