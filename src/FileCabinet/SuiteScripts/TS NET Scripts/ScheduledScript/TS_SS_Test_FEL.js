 ...
 */
define(["N/record", "N/file", "N/email", "N/encode", "N/search", "N/https", "N/log", "N/config"], (record, file, email, encode, search, https, log, config) => {
    var recordtype = '';
    var FOLDER_PDF = 1020; //SB: 1020
    var FOLDER_EXCEPTIONS = 1019; //SB: 1019
    var internalId = '';
    var userId = '';
    ...
    const execute = (context) => {
        try {
            ...
            //var devolucion = getcredentials.wsurl + ' -- ' + getcredentials.username + ' --- ' + getcredentials.password + ' --- ' + getcredentials.recipients;
            ...
            log.debug('Debug-identifydocument', identifydocument);
            ...
            log.debug('request 2', request.request);
            ...
            log.debug('existDocument', existDocument);
            ...
            if (existDocument.codigo == '0') {
                ...
                log.debug('Debug2', statustrasanction);
            }
            ...
            //             log.debug('Send-Email', internalId + ' - ' + sendemail);
            //         } else {
            //             // //sendEmail(false, arrayheader, arraybody, array);
            //             // logError(internalId, userId, docstatus2, 'Error en envío de email');
            //             // result.success = false;
            //             // result.message = "Failure";
            //             log.debug('Error', 'Error en envío de email' + ' - ' + docstatus2);
            //         }
            //     } else {
            //         // //sendEmail(false, tranID, internalId, array);
            //         // logError(internalId, userId, docstatus2, 'Error en generación de archivos');
            //         // result.success = false;
            //         // result.message = "Failure";
            //         log.debug('Error', 'Error en generación de archivos' + ' - ' + docstatus2);
            //     }
            // } else if (send.responsecode == '1033') {
            //     // logError(internalId, userId, docstatus1, send.response);
            //     // result.success = false;
            //     // result.message = "Failure";
            //     log.debug('Error-ResponseCode', docstatus1);
            // } else {
            //     var filetxt = generateFileTXT(request.numbering, request.request, array);
            ...
            //     // result.success = false;
            //     // result.message = "Failure";
            //     log.debug('Error-Else', res + ' - ' + docstatus1);
            // }
        } catch (e) {
            // logError(internalId, userId, docstatus2, e);
            // result.success = false;
            // result.message = "Failure";
            log.debug('Error-execute', docstatus2 + ' - ' + e);
        }
    }
    ...
    function getIdentifyDocument(documentid) {
        try {
                ...
                columns:
                    [
                        ...
                        search.createColumn({
                            ...
                            label: "4Document Type"
                        }),
                        //search.createColumn({ name: "custbody_psg_ei_status", label: "E-Document Status" })
                    ]
            });
            ...
            log.error('Error-getIdentifyDocument',);
        }
    }
    ...
    function getDocumentPDF(username, password, codCPE, numSerieCPE, numCPE, token, url, array) {
        ...
        try {
            ...
            //logError(array[0], array[1], 'Error-getDocumentPDF', e.message);
            log.error('Error-getDocumentPDF', e)
        }
    }
    ...
    function getDocumentXML(username, password, codCPE, numSerieCPE, numCPE, token, url, array) {
        ...
        try {
            ...
            // logError(array[0], array[1], 'Error-getDocumentXML', e.message);
            log.error('Error-getDocumentXML', e)
        }
    }
    ...
    function getDocumentCDR(username, password, codCPE, numSerieCPE, numCPE, token, url, array) {
        ...
        try {
            ...
            //logError(array[0], array[1], 'Error-getDocumentCDR', e.message);
            log.error('Error-getDocumentCDR', e);
        }
    }
    ...
    function sendDocument(username, password, token, url, request, array) {
        ...
        try {
            ...
            //logError(array[0], array[1], 'Error-sendDocument', e.message);
            log.error('Error-sendDocument', e)
        }
    }
    ...
    function createRequest(documentid, array) {
        ...
        try {
            var searchLoad = search.create({
                ...
                columns:
                    [
                        ...
                        // MEMO
                        search.createColumn({ name: "memo", label: "54 MEMO" })
                    ]
            });
            var searchResult = searchLoad.run().getRange({ start: 0, end: 1 });
            // IDE---------------------------------------------------------------------------------------------------------------------
            ...
            log.debug('id_location', id_location);

            log.debug('column21', column21);
            log.debug('column22', column22);
            ...
            log.debug('objPromocion', objPromocion);
            ...
            //logError(array[0], array[1], 'Error-createRequest', e.message);
            log.error('Error-createRequest', e)
        }
    }
        ...
        try {
            ...
            for (var i = 0; i < linecount; i++) {
                ...
                if (itemtype == 'InvtPart' || itemtype == 'Service' || itemtype == 'NonInvtPart') {
                    ...
                    // var itemtype_discount = openRecord.getSublistValue({ sublistId: 'item', fieldId: 'itemtype', line: i + 1 });
                    if (anydiscountline == 'any') {
                        ...
                        //log.debug('TypeOf', typeof tax1amt_discount_line);
                    ...
                    if (itemtype == 'NonInvtPart') {
                        ...
                        jsonTotalImpuestos.push({
                            idImpuesto: '7152',
                            montoImpuesto: montoImp.toString(),
                            tipoAfectacion: tipoAfectacion,
            ...
            if (jsonCargoDescuento.length != 0) {
                ...
                log.debug('dsctos con promociones globales', 'dsctos con promociones globales');
                ...
                log.debug('jsonReturn.totaldescuentos', jsonReturn.totaldescuentos);
                log.debug('jsonReturn.cargodescuento', jsonReturn.cargodescuento);

            }

            return jsonReturn;
        } catch (e) {
            log.error('Error-getDetail', e)
        }
    }
    ...
    function createRequestCreditMemo(documentid, array) {
        ...
        try {
            ...
            //logError(array[0], array[1], 'Error-createRequestCreditMemo', e.message);
            log.error('Error-createRequestCreditMemo', error);
        }
    }
    ...
    function getDetailCreditMemo(documentid, array) {
        ...
        try {
            ...
            //logError(array[0], array[1], 'Error-getDetailCreditMemo', e.message);
            log.error('Error-getDetailCreditMemo', error);
        }
    }
    ...
    function createRequestDebitMemo(documentid, array) {
        ...
        try {
            ...
            //logError(array[0], array[1], 'Error-createRequestCreditMemo', e.message);
            log.error('Error-createRequestDebitMemo', error);
        }
    }
    ...
    function getDetailDebitMemo(documentid, array) {
        ...
        try {
            ...
            //logError(array[0], array[1], 'Error-getDetailCreditMemo', e.message);
            log.error('Error-getDetailDebitMemo', error);
        }
    }
    ...
    function generateFileTXT(namefile, content, array) {
        try {
            ...
            log.error('Error-generateFileTXT', e)
        }
    }
    ...
    function generateFilePDF(namefile, content, array) {
        try {
            ...
            log.error('Error-generateFilePDF', e);
        }
    }
    ...
    function generateFileXML(namefile, content, array) {
        try {
            ...
            log.error('Error-generateFileXML', e);
        }
    }
    ...
    function generateFileCDR(namefile, content, array) {
        try {
            ...
            log.error('Error-generateFileCDR', e);
        }
    }
    ...
    function generateFileJSON(namefile, content, array) {
        try {
            ...
            log.error('Error-generateFileJSON', e);
        }
    }
    ...
    function openCredentials(array) {
        try {
            ...
            log.error('Error-openCredentials', e);
        }
    }
    ...
    function getUnit(itemid) {
        ...
        try {
            ...
            //var unitstype = getunit.unitstype[0].text;
            var unit = getunit.custitem_pe_cod_measure_unit;
            // if (unitstype === "UM Paquetes") {
            //     unit = 'PK'
            // } else if (unitstype === "UM Bolsas") {
            //     unit = 'BG'
            // } else if (unitstype === "UM Cajas") {
            //     unit = 'BX'
            // } else if (unitstype === "UM Servicios") {
            //     unit = 'ZZ'
            // } else { //UM Bienes
            //     unit = 'NIU'
            // }
    ...
    function openFulfillment(ov) {
        ...
        try {
            ...
            log.error('Error-openFulfillment', e);
        }
    }
