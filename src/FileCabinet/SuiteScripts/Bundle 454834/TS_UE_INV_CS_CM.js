...
define(['N/log', 'N/search', 'N/record', 'N/runtime', 'N/redirect', 'N/url', 'N/https', 'N/ui/serverWidget'], (log, search, record, runtime, redirect, url, https, serverWidget) => {
        ...
        if (eventType === context.UserEventType.CREATE || eventType === context.UserEventType.COPY) {
            ...
            try {
                ...
                if (objRecord.type != CREDIT_MEMO) {
                    ...
                    let userClassification = search.lookupFields({ type: search.Type.EMPLOYEE, id: userObj.id, columns: ['department', 'class', 'location', 'custentity_pe_document_number'] });
                ...
                if (objRecord.type == INVOICE) {
                    ...
                    //     log.debug('linecount', 'Lines: ' + linecount);
                    ...
                    //         }
                    //     }
                    //     log.debug('Count', jsonLines.length);
                    //     let lines = 0;
                    //     let vueltas = 50;
                    //     let count = jsonLines.length;
                    //     let linecount2 = objRecord.getLineCount({ sublistId: 'item' });
                    //     if (linecount2 > count) {
                    //         for (let j = 0; j < vueltas; j++) {
                    //             let linecount3 = objRecord.getLineCount({ sublistId: 'item' });
                    //             lines = linecount3;
                    ...
                    //                 }
                    //             }
                    //             lines = objRecord.getLineCount({ sublistId: 'item' });
                    //             lines = lines - count;
                    //             if (lines == 0) {
                    //                 break;
                    //             }
                    ...
                    //     }
                    // } catch (error) {
                    //     log.error('Error-beforeLoad-' + INVOICE, error);
                    // }
        ...
        if (eventType === context.UserEventType.CREATE) {
            ...
            try {
                if (objRecord.type == VENDOR_BILL) {
                    ...
                    const serie_cxp = objRecord.getValue({ fieldId: 'custbody_pe_serie_cxp' });
                    ...
                    try {
                        ...
                        if (cashCsv.length > 0) {//!: Caso de prueba no generación de correlativo por csv
                            let peNumber = objRecord.getValue({ fieldId: 'custbody_pe_number' });
    ...
    function getEDocument(tranid) {
        try {
            ...
            log.error('Error en getEDocument', e);
        }
    }
